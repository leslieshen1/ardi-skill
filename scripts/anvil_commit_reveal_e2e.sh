#!/usr/bin/env bash
# Full on-chain commit-reveal + VRF + inscribe e2e on Anvil
set -euo pipefail
cd "$(dirname "$0")/.."
...
ANVIL_PORT=8547
ANVIL_PID_FILE=/tmp/ardi_commit_reveal_anvil.pid
...
echo
echo "==> Running on-chain commit-reveal e2e"
...
coordinator/.venv/bin/python scripts/anvil_commit_reveal_e2e.py
